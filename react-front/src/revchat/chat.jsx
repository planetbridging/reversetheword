    ...
    render(){
        return (
            <div className="windowBanner"> 
                ...
                {this.renderWords()}
                <div className="footer"><input value={this.state.value} onChange={this.handleChange} type="text" placeholder="hello"/> <button onClick={() => this.btnSend() }>Send</button></div>
            </div>
        )
    }
    ...
    renderWords = () =>{
        ...
        if(tmpLst.length > 0){
            ...
            const listItems = tmpLst.map((pair) =>
                <div className="uiq_tr"><p class="uiq_item">{pair[0]}</p><p class="uiq_item">{pair[1]}</p></div>
            );

            return <div className="uiq_tbl_same_bg">
                    {listItems}
            </div>;
        }else{
            return <div className="uiq_tbl_same_bg">
                    <div className="uiq_tr"><p className="uiq_item">Send to see the reverse</p><p className="uiq_item">the reverse will show here</p></div>
            </div>; 
        }
    }
